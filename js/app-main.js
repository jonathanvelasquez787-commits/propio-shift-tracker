...
// (ver el comentario de mountApp() en boot.js) — nunca antes.
    ...
    const FINANCE_CUSTOM_CATEGORY_PALETTE = ['102,226,165', '255,209,92', '125,140,255', '47,213,255', '255,107,114', '139,123,255', '255,111,143', '88,232,200'];
    ...
    // dólares.
    function usdCeilFromCurrencyAmount(currencyAmount, exchangeRate) {
    ...
    function higherRateBonusForCalls(sourceCalls) {
...
function dayNoteCategoryMeta(value) {
...
// arriba.
const REPORTS_MONTHLY_PAGE_SIZE = 6;
...
function effectiveHistoryStartDate() {
...
// que appConfirm).
function openCalYearGoalEditor() {
...
function useAutoCalYearGoalModal() {
...
function openHistoryStartDateEditor() {
...
function saveCalHistoryStartModal() {
  const input = document.getElementById('calHistoryStartDateInput');
...
function useAutoCalHistoryStartModal() {
  settings.historyStartDateOverride = '';
  saveSettingsOnly();
  closeModal('calHistoryStartModal');
  renderCalYear();
...
function syncCalViewVisibility() {
...
function openCalMonthFromYear(monthIndex) {
  const today = todayRange();
  const year = today.getFullYear() + calYearOffset;
  calMonthOffset = (year - today.getFullYear()) * 12 + (monthIndex - today.getMonth());
  calViewMode = 'month';
  syncCalViewVisibility();
  renderCalendarMonth();
}

function backToCalYearView() {
  calViewMode = 'year';
  syncCalViewVisibility();
  renderCalYear();
}
...
function renderReportsMonthlyChart() {
...
function effectiveDailyGoal(dayKey = getActiveDayKey()) {
...
function reportsCycleProjection() {
  const cycle = cycleEarningsForOffset(0);
  const today = todayRange();
...
function syncFinanceRowTargetFields(source) {
...
function financeConvertedNote(usdAmount) {
...
function financeConvertedInline(usdAmount) {
  const text = convertedAmountText(usdAmount);
...
function financeSwapPriority(catIdIgnored, rowId, direction) {
...
function financeFindRow(catId, rowId) {
  const cat = settings.financeCategories[catId];
...
// asignado).
function financeApplySignedAmount(catId, rowId, rawAmount, sign) {
...
function financeRunAutoFillCascade() {
...
function updateFinanceAutoFillSetting(checked) {
...
function financeResetRow(catId, rowId) {
  const row = financeFindRow(catId, rowId);
  if (!row) return;
...
function financeToggleManualPaid(catId, rowId, checked) {
  const row = financeFindRow(catId, rowId);
  if (!row) return;
...
function financeDeleteRow(catId, rowId) {
  const row = financeFindRow(catId, rowId);
  if (!row) return;
...
function openFinanceRowModal(kind, existingRow = null) {
...
function selectFinanceCategoryChip(catId) {
  const categoryInput = document.getElementById('financeRowCategoryInput');
...
function openFinanceNewCatPanel(catId = null) {
...
function closeFinanceNewCatPanel() {
  const panel = document.getElementById('financeRowNewCatPanel');
...
function createFinanceCategory() {
  const nameInput = document.getElementById('financeNewCatNameInput');
  const emojiInput = document.getElementById('financeNewCatEmojiInput');
  const colorInput = document.getElementById('financeNewCatColorInput');
...
function financeDeleteCategory(catId) {
...
function updateFinanceTotalsScrollHint() {
