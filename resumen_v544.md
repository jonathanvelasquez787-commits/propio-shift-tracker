...
- v544 — Mismo rediseño para el tema OSCURO + el bloque nuevo de "Proyección
  ...
  la derecha. Lo que SÍ revierte parcialmente una decisión anterior, y quedó
  aprobado con el mockup a la vista: el "modo oscuro plano" de v532 dejaba el
  fondo de página y las tarjetas del mismo color y varios separadores en
  `transparent`; ahora el fondo baja un escalón y esos separadores se
  encienden. Sigue sin degradados: la decisión de "plano" se mantiene, lo que
  cambia es que la tarjeta se distinga del fondo.
...
- **Los dos temas se afinan con una capa, no en su origen.**
  ...
  `:root.theme-dark` porque `app.html` (congelado, generado) no remapea sus
  colores: los reemplaza regla por regla y deja fuera lo que nadie enumeró.
  Funciona y es estable — `:root.theme-X` le gana por especificidad a
  `html.theme-X` y por orden a `:root:not(.theme-dark)` dentro del `@media` —
  pero el lugar correcto es el bloque de tokens de cada tema, y se unifica en la
  Fase 3. Lo mismo con las 4 reglas de tema oscuro de las páginas públicas, que
  hoy viven al final de `css/tokens.css` en vez de en `landing.css`/`legal.css`.
...
- La Proyección del ciclo de Inicio muestra SIEMPRE el ciclo actual, igual que
  ...
  aceptar el offset, y eso vive en el script congelado.
...
- **Dónde vive y por qué.** El bloque no puede ir en `app.html`: ese archivo se
  genera desde el congelado y `build_app_html.py` verifica que revertir los 4
  cambios devuelva el v539 byte por byte. Así que `boot.js` lo inyecta después
  de montar, igual que ya hace con el panel de cuenta y la tarjeta de borrar
  cuenta. No calcula nada nuevo: llama a `reportsCycleProjection()`,
  `cycleGoalPace()` y `effectiveCycleGoal()`, las mismas funciones que alimentan
  Reportes. Antes de inyectar comprueba que las 7 globales que necesita existan;
  si alguna falta, el bloque simplemente no aparece.
...
- No se tocó: ni una línea del script de la app, ni `app.html`, ni
  ...
  `tools/build_app_html.py`, ni el SQL, ni la landing. El modo claro de v543 no
  cambia salvo por las reglas de etiquetas/números, que ahora son compartidas.
- Verificado acá: `node --check` limpio en `js/boot.js` (como módulo ESM);
  llaves balanceadas en `css/app-shell.css` (76/76); las 13 reglas de
  `:root.theme-dark` y las 97 de `:root.theme-light` no se pisan entre sí porque
  las dos clases son excluyentes; los 19 anclajes que `boot.js` necesita de la
  app (`#earningsCycleView`, `.gh-info-row`, `#cycleRangeBadge`,
  `#earningsAccordion`, los 3 botones de ciclo, `[data-earnings-mode]`,
  `data-nav-page`, `.app-page[data-page="home"]` y las 11 clases de
  `.fin-preview`) existen tal cual en `app.html`; las 7 globales que usa son
  todas declaraciones de función de nivel superior, que sí quedan en `window`.
- **Sin probar todavía**: el aspecto real del oscuro y los números de la
  proyección solo se pueden juzgar con la app corriendo y con datos de un ciclo
  a medias. Prueba mínima: entrar en oscuro, abrir Ganancias de hoy → Ciclo,
  ver las dos filas, tocar "Ver Reportes →" (debe llevar a Reportes) y después
  "‹ Anterior" (el bloque debe desaparecer). Tampoco se corrió
  `pytest tests/smoke.py`, que exige Playwright.
...
- **Modo claro de la app, al nivel de la landing.** El pedido fue explícito:
  ...
  etiquetas en mayúsculas finas donde acompañan a un número y `tabular-nums`
  en relojes, contadores y montos.
...
- No se tocó: ni una línea del script de la app, ni `app.html`, ni el
  congelado, ni `tools/build_app_html.py`, ni el SQL, ni
  `js/auth.js`/`js/landing.js`/`js/sync.js`.
- Verificado acá: `node --check` limpio en `js/theme-toggle.js` y
  `js/landing-ui.js`; llaves balanceadas en los dos CSS; etiquetas de
  `index.html` balanceadas; 37 ids, cero duplicados, y los 31 que busca
  `landing.js` más `previewClock`/`year` siguen presentes; los 3
  `data-reveal-for` apuntan a ids que existen; todos los enlaces internos de
  `index.html` resuelven a un archivo real.
...
- **Azul en modo claro (gate, tarjetas de arranque y banner de conflicto).**
  ...
  Fix en `css/app-shell.css`: remapear `--panel-navy` a blanco en claro. Una
  línea cubre los cinco casos y cualquiera futuro — los elementos que ya tenían
  su propia rama de claro no cambian, porque esa regla sigue ganando por
  especificidad.
- **El gate ya no depende del tema para verse.** `syncThemeClass()` corre en la
  ...
  desaparecían. Ahora heredan el color del gate.
...
- No se tocó: ni una línea del script de la app, ni `app.html`, ni
  `tools/build_app_html.py`, ni el SQL.
