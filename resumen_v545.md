...
- El protocolo de LAYOUT, el protocolo de COMENTARIOS, el protocolo de
  ...
  permanente (no crecen con cada versión), se mantienen siempre.
...
- v543 — Rediseño del tema CLARO de la app para que se parezca a la landing:
  ...
  un listener.
- v544 — Mismo rediseño para el tema OSCURO + el bloque nuevo de "Proyección
  del ciclo" en Inicio. Protocolo cumplido: mockup visual con las 2 vistas
  (escritorio/celular) MÁS un alternador Oscuro/Claro para comprobar que son el
  mismo diseño en dos temas, y el bloque nuevo marcado aparte; aprobado
  explícitamente ("Dale así"). El bloque nuevo no inventa lenguaje visual:
  reutiliza 1:1 `.fin-preview` de "Próximos pendientes", con el mismo enlace a
  la derecha.
...
LECCIÓN 4 (agregada en v544) — al escribir reglas de tema en
`css/app-shell.css`, que carga DESPUÉS del `<style>` de la app, una regla sin
prefijo de tema le gana por orden a los media queries mobile de `app.html` con
la misma especificidad. Si esa regla toca `font-size`, se pierde la
compactación de mobile y aparece un desborde donde no había nada. Por eso las
etiquetas en mayúsculas de v543/v544 solo fijan tamaño donde el elemento NO
tiene una regla propia dentro de un `@media (max-width: …)` (ver
`.month-card-amt-label` y `.wk-total-metric-label`, que quedaron fuera de esa
lista a propósito).
...
3. Buscar con grep, dentro de la sección en cuestión, cualquiera de estos
   ...
   - `overflow-x: auto` con scrollbar oculta cubriendo un texto/dato que el
     ...
     pida explícitamente.
   - Reglas sin prefijo de tema en `css/app-shell.css` que fijen `font-size`
     sobre un elemento con compactación mobile propia — ver LECCIÓN 4.
...
- Esto aplica sin excepción a montos nuevos que se agreguen en cualquier
  ...
  (Ganancias de hoy/del ciclo)/`renderCalYear`/etc., y también el bloque de
  Proyección del ciclo que inyecta `js/boot.js`.
...
futura debe operar bajo esto, sin excepción:
...
- **`app.html` es el ÚNICO archivo de la app y la única fuente de verdad.**
  ...
  ni un script que genere `app.html` a partir de otro archivo, ni ninguna
  verificación "byte por byte". Ese sistema existió entre v540 y v544 y se
  retiró por completo en v545 porque exigía que el usuario ejecutara un
  script en terminal — justo lo que no puede ni quiere hacer.
- **Cualquier cambio a `app.html` (o a cualquier otro archivo del repo) lo
  ...
  el editor de GitHub" — eso es pedirle que edite código, que es justo lo
  que esta regla prohíbe.
- Claude entrega el archivo **completo y ya editado**, listo para reemplazar
  tal cual en GitHub. El usuario lo sube reemplazando el existente por la
  interfaz web de GitHub (arrastrar el archivo nuevo encima del viejo, o el
  ícono de lápiz para pegarlo entero) y confirma con "Commit changes" — nunca
  por terminal, nunca instalando nada, nunca con `git`.
- Si el archivo es muy grande para subirlo o pegarlo directo en el chat (el
  usuario ya reportó que "excede tokens" con `app.html` completo), la salida
  es que el usuario pegue el contenido como texto/documento en el chat (no
  como archivo adjunto) cuando el tamaño lo permita, o que Claude trabaje con
  la última versión completa que ya tenga en el historial de la conversación.
  Nunca se le pide al usuario que reduzca o divida el archivo él mismo.
- Cualquier script de apoyo (generadores, verificadores, migraciones de este
  `.md`, etc.) es responsabilidad exclusiva de Claude: si hace falta uno para
  producir un resultado, Claude lo ejecuta él mismo con sus herramientas —
  nunca se entrega como algo que el usuario deba correr.
- Si en el futuro se reintroduce alguna prueba automática (por ejemplo
  `tests/smoke.py`, que hoy sigue existiendo de forma simplificada), es una
  herramienta que Claude puede correr o interpretar por su cuenta cuando
  tenga el entorno disponible — nunca una obligación ni un paso manual del
  usuario.

---

**ROADMAP A PRODUCCIÓN — login, nube, modularización y protección del código**
(Fases 0, 1 y 2 cerradas en v540-v542; Fase 0 revertida parcialmente en v545,
ver abajo. Este bloque se mantiene hasta que las 5 estén cerradas; al cerrar
cada fase se borra su detalle y se deja una línea.)
...
FASE 0 — **CERRADA en v540, revertida parcialmente en v545.** Originalmente
se había congelado `reference/propio_shift_tracker_Fixed_v539.html` +
`tools/build_app_html.py` (generador de `app.html` con verificación byte por
byte) + `tests/smoke.py` corriendo contra dos objetivos (el congelado y el
generado). En v545 se retiró todo ese sistema por completo: el usuario no
programa y no puede/quiere ejecutar scripts. Hoy `app.html` es el único
archivo, se edita directo (ver FLUJO DE TRABAJO CON ESTE REPOSITORIO arriba),
y `tests/smoke.py` quedó reescrito para correr una sola vez, directo contra
`app.html`, sin ningún archivo de referencia ni verificación de generación.
...
- **Nota importante para esta fase, desde v545**: esta modularización implica
  correr una herramienta de build (Vite) — algo que el usuario no puede/quiere
  hacer. Cuando llegue el momento de encarar esta fase, hay que resolver
  primero CÓMO se entrega el resultado sin que el usuario tenga que ejecutar
  ningún comando (ej. Claude corre el build por su cuenta con sus propias
  herramientas y entrega los archivos finales ya compilados, o se evalúa si
  vale la pena esta fase dado que rompe el patrón de "un solo archivo que se
  edita y se sube tal cual").
- **Andamios que hay que desmontar acá**, todos introducidos por el envoltorio:
  `js/boot.js` monta la app inyectando el contenido de `#appMainScript`,
  `js/sync.js` intercepta `Storage.prototype.setItem`, y `js/boot.js` inyecta a
  mano tres pedazos de interfaz (panel de cuenta en el sidebar, tarjeta de
  borrar cuenta en Ajustes y el bloque de Proyección del ciclo en Inicio).
  Ninguno es permanente: al modularizar, el boot importa la app normalmente, el
  sync se engancha a las funciones de guardado reales y los tres bloques pasan a
  ser HTML de la app como cualquier otro.
- **Dependencia de `boot.js` sobre globales de la app**: usa `window.appConfirm`
  y `window.toast`, y para la Proyección del ciclo además
  `window.reportsCycleProjection`, `window.cycleGoalPace`,
  `window.effectiveCycleGoal`, `window.money`, `window.financeConvertedInline`,
  `window.convertedAmountText` y `window.iconHtml` (declaraciones de función en
  script clásico, que sí quedan en `window`). Al modularizar dejan de ser
  globales: hay que exportarlas. `boot.js` ya chequea que existan antes de
  inyectar nada, así que si alguna desaparece el bloque simplemente no aparece,
  no truena.
- **Deuda de tema que hay que saldar acá** (introducida en v542, ampliada en
  v543 y v544, ver PENDIENTES): TODO el afinado de los dos temas vive hoy en
  `css/app-shell.css` como una capa que le gana por cascada a `app.html`, no en
  su origen. Al unificar el CSS, esas reglas se mudan a los bloques de tokens de
  cada tema y la capa entera se borra — incluidos los dos tokens nuevos
...
- Orden correcto: extraer primero el CSS (bajísimo riesgo), después los módulos
  ...
  al final los `render*` y el wiring.
...
- La verdad primero: **cualquier JS que llegue al navegador se puede leer**. No
  ...
  1. **Minificar y bundlear** y **no publicar sourcemaps**. — pendiente,
     depende de Fase 3, y de resolver cómo se hace sin que el usuario ejecute
     nada (ver nota en Fase 3).
  ...
  3. **Gate de cuenta**: sin login no hay app. — **HECHO en v540** (ver
     `#appMainScript` + `js/boot.js`), sigue vigente tras v545: `app.html` ya
     no se genera desde un congelado, pero el `<script>` principal sigue
     marcado `type="text/plain"` y `boot.js` lo sigue inyectando igual.
...
- **Los dos temas se afinan con una capa, no en su origen.**
  `css/app-shell.css` redefine tokens y ~30 reglas bajo `:root.theme-light` /
  `:root.theme-dark` porque `app.html` no remapea sus colores: los reemplaza
  regla por regla y deja fuera lo que nadie enumeró. Funciona y es estable —
  `:root.theme-X` le gana por especificidad a `html.theme-X` y por orden a
  `:root:not(.theme-dark)` dentro del `@media` — pero el lugar correcto es el
  bloque de tokens de cada tema, y se unifica en la Fase 3. Lo mismo con las 4
  reglas de tema oscuro de las páginas públicas, que hoy viven al final de
  `css/tokens.css` en vez de en `landing.css`/`legal.css`.
...
- **`@supabase/supabase-js` pinchado a 2.45.4 desde jsDelivr.** No hay lockfile
  ...
  gate está escrito sin depender de los tokens de la app.
...
- **`resumen_vXXX.md` menciona versiones de la Fase 0 que ya no existen tal
  cual.** Entradas de ÚLTIMOS FIXES anteriores a v545 (y el propio historial de
  ROADMAP más arriba) describen el archivo congelado y el script generador
  como si siguieran vigentes en el momento en que se escribieron — es
  correcto como historial de decisiones (qué se hizo y por qué en su
  momento), no hace falta "corregirlo" retroactivamente; solo tener presente
  que esa parte del sistema ya no existe desde v545.

**LIMITACIONES CONOCIDAS** (alcance reducido a propósito, no son bugs)

- La Proyección del ciclo de Inicio muestra SIEMPRE el ciclo actual, igual que
  la de Reportes (`reportsCycleProjection()` y `cycleGoalPace()` están fijadas
  al offset 0). Por eso el bloque se esconde solo al navegar a otro ciclo con
  las flechas: detecta el "(actual)" del rótulo `#cycleRangeBadge`. Si algún día
  se quiere la proyección de un ciclo pasado, esas dos funciones tienen que
  aceptar el offset.
- Esa misma proyección se repinta cada 5 s y solo mientras la pestaña Ciclo está
  abierta en Inicio. Recorre los 14 días del ciclo, así que hacerlo cada segundo
  sería el mismo costo que Reportes evita recalculando solo cuando su página
  está visible.
...
- Higher Rate: editar la FECHA o el HORARIO de una ventana congela el bono ya
  ...
  una hora mal tecleada deja el bono viejo pegado además del nuevo.
- Una llamada en vivo (botón "Iniciar/Terminar llamada") sigue redondeando a
  1 min como mínimo (`buildLiveCallCandidate`, `Math.max(1, ...)`). El
  importador y los modales sí aceptan 0 min; ese piso de 1 min se dejó a
...
- Cambiar el ancla del ciclo de pago mueve el inicio de TODAS las semanas
  ...
  "Semana específica". Esas claves se re-anclan solas por el centro de la
  semana vieja, pero si el ancla se mueve varios días una semana personalizada
  puede terminar cubriendo un día distinto en los extremos.

---

**ÚLTIMOS FIXES (máx. 3, los más recientes)**

**v545** — El usuario dejó claro que no programa: no puede ni quiere ejecutar
scripts, editar código a mano, ni verificar nada técnicamente. Se retira por
completo el sistema de "archivo congelado + script generador + verificación
byte por byte" entregado en v540, y se deja anotado el flujo de trabajo
correcto de forma permanente para que ninguna sesión futura lo repita.

- **Se borraron** (por el usuario, con guía de Claude): `tools/build_app_html.py`
  y su reemplazo de aviso, `reference/propio_shift_tracker_Fixed_v539.html`,
  `tools/build_resumen.py` (script de un solo uso ya cumplido), y la carpeta
  `tests/` vieja completa (`smoke.py`, `README.md`, `requirements.txt`).
- **`app.html` pasa a ser el único archivo de la app.** Se edita siempre
  directo: el usuario pega o sube el contenido en el chat, Claude lo modifica
  con sus propias herramientas de archivo, y entrega el archivo completo listo
  para reemplazar en GitHub por la interfaz web (arrastrar encima + "Commit
  changes"). Nunca más instrucciones de "busca X, reemplaza por Y" para que el
  usuario edite código él mismo.
- **`tests/smoke.py` se reescribió desde cero**, sin depender de ningún
  archivo de referencia: mismas 8 pruebas de siempre (carga sin errores en 2
  zonas horarias × 2 anchos, las 6 páginas del sidebar, los 16 modales con
  Tab/Escape, sin desborde horizontal, migración de datos viejos, popup de
  bienvenida en perfil nuevo, gate de sesión bloqueando la app, landing y
  páginas legales) pero corriendo UNA sola vez, directo contra `app.html` — sin
  la parametrización `reference`/`app` ni la prueba de generación que
  comparaba contra el congelado. Sigue siendo una herramienta que Claude puede
  correr por su cuenta cuando tenga el entorno, nunca algo que el usuario deba
  ejecutar.
- **`package.json` perdió los 3 scripts** (`build:app`, `check:app`,
  `test:smoke`) que apuntaban al generador borrado; el resto del archivo queda
  igual.
- **Nueva sección permanente en este `.md`**: "FLUJO DE TRABAJO CON ESTE
  REPOSITORIO", al mismo nivel que los demás protocolos permanentes (LAYOUT,
  COMENTARIOS, DIAGNÓSTICO DE OVERFLOW, MONEDA SECUNDARIA) — deja por escrito,
  para cualquier sesión futura, que el usuario no programa y que todo cambio
  a este repo lo aplica y entrega Claude, nunca el usuario.
- No se tocó ni una línea del script principal de la app, ni su HTML/CSS.
  Este cambio es puramente de organización de archivos del repo y de proceso
  de trabajo.

**v544** — Segunda mitad del rediseño: el modo oscuro al nivel del login, y la
Proyección del ciclo también dentro de Inicio. Mockup con las dos vistas más un
alternador Oscuro/Claro, mostrado y aprobado antes de tocar código. Dos
archivos: `css/app-shell.css` y `js/boot.js`.

- **El fondo baja un escalón, las tarjetas no se mueven.** El fondo de página y
  las tarjetas eran exactamente el mismo `#0d1230`, así que la tarjeta no
  existía como tarjeta. Solo se oscurece `body` a `#080d1f`: ni un color de
  tarjeta, modal o sidebar cambia. Es el mismo arreglo que el claro, al revés.
- **Las líneas por fin se ven.** El grande: `--line` valía
  `rgba(90,105,150,.045)` — 4,5% de opacidad, o sea nada. Chips, campos, pills
  segmentadas, botones del header, separadores de fila y rieles de progreso
  vivían sin contorno. Pasa a `rgba(125,145,215,.16)` y `--line-2` a `.26`, los
  mismos valores que ya usa la landing; `--card-line` sube de `.13` a `.18`.
- **Un solo acento, cian.** `--hover-flip` era `var(--purple)`: los hover
  saltaban a morado aunque todo el tema fuera cian. Se unifica en cian con un
  solo par de tokens (`--hover-flip` / `--hover-flip-rgb`), igual que el claro
  se unificó en índigo.
- **Separadores del modo plano, encendidos.** `.hero-top-row`,
  `.sidebar-brand-block`, `.sidebar-footer` y `.divider` quedaban en
  `transparent` desde el rediseño de modo oscuro plano. Con el resto de las
  líneas ya visibles eran los únicos bordes ausentes. Sigue sin degradados: lo
  "plano" se mantiene, lo que cambia es que la tarjeta se distinga del fondo.
- **Etiquetas y cifras de ancho fijo salen del modo claro.** Las mayúsculas
  finas sobre cada número y `tabular-nums` pasan a reglas sin prefijo de tema:
  el reloj y los contadores dejan de bailar también en oscuro. Con una
  precaución nueva, anotada como LECCIÓN 4 del protocolo de overflow: el tamaño
  solo se fija donde el elemento NO tiene compactación mobile propia, porque
  este archivo carga después del `<style>` y le ganaría a esos media queries.
- **Proyección del ciclo en Inicio.** Dentro de Ganancias del ciclo, entre la
  meta y las pastillas de fechas, con el mismo `.fin-preview` de "Próximos
  pendientes" y su enlace a la derecha — cero lenguaje visual nuevo. Dos filas:
  "Al ritmo de hoy" (proyección, día N de 14, delta contra la meta) y "Para
  llegar a tu meta" (horas productivas por día, con el faltante y el monto
  diario en las dos monedas). El botón "Ver Reportes →" usa `data-nav-page`,
  que la app ya maneja con un listener delegado: no necesitó wiring propio.
- **Dónde vive y por qué.** El bloque no puede ir directo en `app.html` a
  mano cada vez: `boot.js` lo inyecta después de montar, igual que ya hace con
  el panel de cuenta y la tarjeta de borrar cuenta. No calcula nada nuevo:
  llama a `reportsCycleProjection()`, `cycleGoalPace()` y
  `effectiveCycleGoal()`, las mismas funciones que alimentan Reportes. Antes
  de inyectar comprueba que las 7 globales que necesita existan; si alguna
  falta, el bloque simplemente no aparece.
- **Se repinta solo cuando se está viendo.** Esos cálculos recorren los 14 días
  del ciclo, así que hay un refresco de 5 s condicionado a que la pestaña Ciclo
  esté abierta, en Inicio, con el acordeón desplegado — más un repintado
  inmediato al tocar la pastilla Ciclo o las flechas de ciclo. Y como los
  números son siempre del ciclo actual, el bloque se esconde solo al navegar a
  otro, detectando el "(actual)" del rótulo del rango.
- No se tocó: ni una línea del script de la app, ni el SQL, ni la landing.
...
- **Modo claro de la app, al nivel de la landing.** El pedido fue explícito:
  ...
  (1) fondo de página gris muy claro con las tarjetas blancas recuperando su
  borde; (2) un solo acento (índigo `--accent-decorative`) en vez de un azul
  suelto que no era de la paleta; (3) tipografía de datos: etiquetas en
  mayúsculas finas donde acompañan a un número y `tabular-nums` en relojes,
  contadores y montos.
- **Cómo gana la cascada sin tocar `app.html`.** Todo lo nuevo va bajo
  `:root.theme-light X`, que le gana por especificidad y por orden de cascada
  a las reglas de tema claro que ya trae `app.html`. Es fiable porque
  `boot.js` llama a `applyLightThemeDefault()` antes de montar: la clase de
  tema SIEMPRE está puesta en `/app`.
- **Dos tokens nuevos, solo en claro:** `--surface-sunken` (#f8fafc) y
  `--surface-control` (#f1f5f9), para los fondos "hundidos" que en oscuro son
  `rgba(255,255,255,.03..09)` y sobre blanco simplemente no existían.
- No se tocó: ni una línea del script de la app, ni el SQL, ni
  `js/auth.js`/`js/landing.js`/`js/sync.js`.
