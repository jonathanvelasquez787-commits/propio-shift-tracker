...
- **Deuda de tema que hay que saldar acá** (introducida en v542, ver PENDIENTES):
  el tema claro de la app no remapea `--panel-navy`, lo reemplaza regla por
  regla; hoy hay un parche en `css/app-shell.css` que sí lo remapea. Al unificar
  el CSS, ese token se define una sola vez por tema y el parche se borra.
...
- **El tema claro de la app se arregla con un parche, no en su origen.**
  `css/app-shell.css` remapea `--panel-navy` a blanco bajo `html.theme-light`
  porque `app.html` (congelado, generado) no lo hace: reemplaza ese color regla
  por regla. Funciona y cubre lo que faltaba, pero el lugar correcto es el
  bloque de tokens del tema claro — se unifica en la Fase 3. Lo mismo con las 4
